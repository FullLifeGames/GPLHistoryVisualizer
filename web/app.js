...
function renderZeitstrahlWidgetReadout(events) {
  const results = document.querySelector("#zeitstrahl-today-results");
  const label = document.querySelector("#zeitstrahl-date-label");
...
function pokemonTypeLabel(type) {
  const label = t(state.language, `pokemonTypes.${type}`);
  return label === `pokemonTypes.${type}` ? type : label;
}
...
function teamDuelSideHtml(side) {
  ...
  const rows = side.mons
    .map((mon) => {
      const types = mon.types.map((type) => `<span class="team-duel-type">${escapeHtml(pokemonTypeLabel(type))}</span>`).join("");
...
// lineup reads as a list of roles instead of a wall of text. Matchup
// contributions against the opposing roster take precedence.
...
function teamDuelSixHtml(sheet, genA, genB) {
  const sixA = hypotheticalSix(sheet.a, genA, sheet.b);
  const sixB = hypotheticalSix(sheet.b, genB, sheet.a);
  ...
  return `
    <div class="team-duel-six">
      <div class="team-duel-sim-head">
        <h3>${escapeHtml(t(state.language, "teamDuel.sixTitle"))}</h3>
...
function renderTeamDuel() {
  ...
  const hosts = {
    ...
    six: document.querySelector("#team-duel-six"),
