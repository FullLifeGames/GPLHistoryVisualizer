...
All five games are generated from the normalized CSVs — nothing is hand-authored and nothing is official. The daily Kader-Raten puzzle is seeded by the visitor's local date, so users in different time zones can see the day roll over at different moments; rosters need at least six known Pokémon to qualify, so seasons with thin roster coverage never appear, and the free-play mode draws session-seeded rosters that never touch the daily streak. The Tipp-Spiel only offers matches that were actually played, decided, and fully scored (the same forfeit/unresolved exclusions as the Orakel), and its Elo forecast comes from the client-side full-archive chronology (K=32, start 1500) — computed, not official; both sides render in the roster teamsheet look; the recorded Spieltag lineup is shown where `roster_matchdays.csv` covers the match, otherwise the full season roster, and matches where either player has no recorded roster never enter the pool. The Tipp-Spiel's score guess offers the dominant W:0 pattern (1:0 through 6:0) — the handful of odd historic scorelines (like 2:1) simply can never be hit exactly. The Klick-Duell's stats mode compares two careers on a random stat and only pairs people whose values differ. "Wer bin ich?" riddles only people with at least 10 recorded matches. Quiz questions are built from champions, primary final tables, killlists (only trainers whose top killer is unambiguous), draft instances, and head-to-heads with at least 5 meetings and a clear leader; in the "Alle" category the draw is weighted by the square root of each pool's size, so small pools like champions still appear but proportionally less often, and every reveal cites the source rows it was generated from. Only the daily Kader-Raten progress and streak persist in localStorage (`gpl-game-kader-raten-*`); every other score — Tipp-Spiel tally, Klick-Duell streaks, all quiz modes (Endlos, Blitz, Sudden Death) — is session-only and resets on reload. Klick-Duell thumbnails load from `i.ytimg.com` and degrade to text when offline.

## Videos & Zeit (computed)

The Publikums-Geschichte and Zeitstrahl (menu label "Meilensteine") views are computed entirely from `video_archive.csv`, `seasons.csv`, `champions.csv`, `match_highlights.csv`, `records_progression.csv`, and `match_videos.csv` — no new data is collected. Every date shown is a **video upload date** (`published_at`), not a match date: `matches.csv` records no dates, and season start dates come from `seasons.csv` (first season upload at collection time). Season **end** dates are recomputed from `match_videos.csv` because the collected `end_date` can be stale (Season 10's finale aired after collection): per match the most frequent upload date wins (ties resolve to the earliest, which defuses both years-later re-uploads and early announcement videos matched to a final), and the season ends with its finale match where a "Finale" week exists, else with its last match. The monthly charts stack the top 10 groups by the selected metric (uploads, views, likes, or comments) along a selectable breakdown dimension (channel, video category, detected season, or perspective person; ungrouped rows show as "Ohne Zuordnung") and fold everything else into an "Andere" bucket; a channel filter narrows the whole view — chart, breakdown, and anomalies — to one channel; season band labels degrade to short forms (or hover-only titles) on narrow screens. The attention strip and the anomaly lists follow the selected metric: uploads/views compare reach (view z-scores; anomalies for views compare raw views to the channel median), likes/comments compare engagement (the strip switches to `engagement_z_score_peak`, anomalies to like rate or comment rate per view). All anomaly factors are computed, not official: each video against the **same channel's median**, over videos with at least 500 views on channels with at least 5 qualifying videos — a low factor can simply mean disabled comments, and all counts are today's YouTube stats. The attention strip needs `match_highlights.csv` rows for the chosen season. The Zeitstrahl shows record events only when the record changed hands (a holder improving their own record and each record's initial row are skipped) and only when the match resolves to a dated video via `match_videos.csv` — unresolvable record hand-offs are omitted rather than mis-dated. Top-video events rank the 3 most-viewed uploads per calendar year with today's view counts (a late view surge re-ranks history); milestone events count archive rows in upload order, so they shift if videos are added retroactively. The optional dismissible "An diesem Tag" teaser chip on the default view from the design spec was deliberately not built. The season/division toolbar is hidden on both views because they are archive-scope.

## Saison-Erzählungen (computed/simulated)

- **Das Titelrennen ist eine Simulation.** `title_odds.csv` kommt aus einem Monte-Carlo (`gpl-history title-odds`, Standard 1000 Durchläufe, Seed 42, beides pro Zeile dokumentiert): 3/1/0-Punkte ohne die offiziellen Punktabzüge (11 von 215 offiziellen Tabellenzeilen weichen deshalb real ab), Sieg-Wahrscheinlichkeiten aus zum jeweiligen Spieltag eingefrorenen Karriere-Elo-Werten, simulierte Spiele kennen kein Unentschieden. Gleichstände brechen wie die offiziellen Tabellen nach Punkten, dann nach dem aus bekannten Ergebnissen aufgelaufenen Differential, erst der Rest fällt an den Zufall — am letzten Spieltag reproduziert das die offizielle Rangfolge. `p_first` heißt an Spieltags-Checkpoints: Platz 1 der regulären Saison der jeweiligen Liga; `p_playoffs` zeigt die Qualifikationswahrscheinlichkeit (Teilnehmerzahl aus den echten Playoff-Spielen abgeleitet). Bei den Playoff-Saisons 6 und 10 geht das Rennen danach in Runden-Checkpoints (VR/VF/HF/Finale) weiter: dort ist `p_first` die Titelwahrscheinlichkeit über den **echten Turnierbaum** (Slots aus Sieger/Verlierer-Herkunft der tatsächlichen Playoff-Spiele; ungespielte Runden per eingefrorenem Elo simuliert) — vor dem Halbfinale können also nur Spieler des realen Bracket-Pfads Titelchancen haben. Der „rechnerisch entschieden"-Marker erscheint nur bei vorzeitiger Gewissheit, nie am letzten Checkpoint. Die Chart-Legenden zeigen alle Spieler, deren Wahrscheinlichkeit je mindestens 5 % erreichte. Die CSV ist bewusst **nicht** Teil von `check-generated` (Simulationskosten) und wird per README-Kommando nach Datenänderungen regeneriert. Jede Darstellung trägt Simulation-Badge und gestrichelte Linien.
- **Die Season Story erzählt aus Daten, nicht aus Prosa.** Alle Sätze sind i18n-Templates über `matches.csv`, die rekonstruierte Tabellenhistorie (gleiche Regeln wie die Zeitreise), `match_highlights.csv` und `title_odds.csv`; der „rechnerisch entschieden"-Beat ist als Simulation gebadged, und jeder Beat zitiert seine Quellzeilen. Season Story und GPL Wrapped sind Kapitel des Saison-Hubs (Unter-Tabs „Story" und „Wrapped"); die Chronik (View-ID `zeitreise`) verlinkt nicht (sie hat keine gewählte Saison). Die Award-Beats folgen den `awards.csv`-Regeln und sind berechnet, nicht offiziell.
- **GPL Wrapped:** Auszeichnungs-Karten folgen den `awards.csv`-Regeln und sind berechnet, nicht offiziell; die Karte „MVP-Pokémon" aus der Design-Spec wurde zu MVP + Kill-Leader, weil es kein Saison-Pokémon-MVP in den Awards gibt. Der PNG-Export (`gpl-history wrapped-cards`, optionales Extra `pip install .[wrapped]`) ist deutschsprachig, nutzt die Roster-Hintergrundbilder und ist nicht drift-geprüft (Pillow-Bytes variieren zwischen Versionen); Karriere-Wrapped ist DOM-only ohne PNG.

## Zeitmaschinen-Duell (computed/simulated)

- **Die Gen-Daten kommen zur Laufzeit von unpkg.** Das Duell lädt `@pkmn/dex` (~1,8 MB) und `@pkmn/data` als CDN-Bundles; offline oder mit blockiertem unpkg degradiert die Ansicht zu reinen Kadernamen ohne Statuswerte (gleiches Fallback-Prinzip wie die Sprites). Pokémon ohne Showdown-ID-Zuordnung erscheinen unter „Ohne Gen-Daten".
- **Jede Seite rechnet mit den Regeln ihrer eigenen Generation** (`ROSTER_SEASON_GENERATION`: S1–S3 Gen 6, S4–S6 Gen 7, S7–S9 Gen 8, S10 Gen 9); Cross-Era-Paare tragen einen expliziten Generationen-Hinweis. Kader speisen sich aus derselben zusammengeführten Basis wie die Kaderübersicht (manuelle Zuordnungen + `team_rosters.csv` + Killlisten-Ableitung, daher auch S1/S2), und jede Kader-Phase (Hin-/Rückrunde, Hauptrunde/Playoffs) ist ein eigener wählbarer Kader.
- **Die Typen-Bilanz rechnet Fähigkeits-Effekte ein:** Jede Fähigkeit, die eine Spezies haben KANN, zählt — Immunitäten (Schwebe, Voltabsorber) als immun, halbierende Fähigkeiten (Speckschicht, Hitzeschutz) als Resistenz —, unabhängig davon, welche Fähigkeit im historischen Match tatsächlich gespielt wurde. Angezeigt wird pro Angriffstyp die Netto-Bilanz (Resistenzen + Immunitäten − Schwächen) als Stärke/Schwäche/Neutral.
- **Die „Hypothetische 6er-Aufstellung" ist ein berechneter Vorschlag**, gegen den konkreten Gegner-Kader aufgestellt und zwischen sehr effektiven Treffern und STAB-Resistenz ausbalanciert (dazu Deckung eigener Schwächen und Typ-Vielfalt; Basiswerte nur als Tiebreak). Cross-Era-Paare bewerten beide Richtungen unter den Regeln der eigenen Generation.
- **Der „Hypothetische Ausgang" ist eine Simulation**, kein historisches Ergebnis: Elo-Stände beider Trainer am Ende der jeweils gewählten Saison (nicht heutige Werte), umgerechnet per Logistik-Formel; gebadged und gestrichelt abgesetzt. Der zugehörige Spoiler „Tatsächlicher Ausgang" zeigt bei Kadern derselben Saison die echten direkten Duelle dieser Saison, bei Cross-Era-Paaren die Karriere-Bilanz der beiden Trainer.

## Zeitreise View (menu label "Chronik")
