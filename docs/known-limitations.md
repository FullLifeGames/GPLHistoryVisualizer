...
- **Das Titelrennen ist eine Simulation.** `title_odds.csv` kommt aus einem Monte-Carlo (`gpl-history title-odds`, Standard 1000 Durchläufe, Seed 42, beides pro Zeile dokumentiert): 3/1/0-Punkte ohne die offiziellen Punktabzüge (11 von 215 offiziellen Tabellenzeilen weichen deshalb real ab), Sieg-Wahrscheinlichkeiten aus zum jeweiligen Spieltag eingefrorenen Karriere-Elo-Werten, simulierte Spiele kennen kein Unentschieden, und Punktgleichstände werden pro Simulation zufällig sortiert (real entschied das Differential — die 53/47-Aufteilung am letzten S10-Spieltag ist genau dieser Unterschied). `p_first` heißt: Platz 1 der regulären Saison der jeweiligen Liga — bei den Playoff-Saisons 6 und 10 also Conference-/Hauptrundensieg, nicht der Playoff-Titel; dafür zeigt `p_playoffs` die Qualifikationswahrscheinlichkeit, mit der Teilnehmerzahl aus den echten Playoff-Spielen abgeleitet. Die CSV ist bewusst **nicht** Teil von `check-generated` (Simulationskosten) und wird per README-Kommando nach Datenänderungen regeneriert. Jede Darstellung trägt Simulation-Badge und gestrichelte Linien.
