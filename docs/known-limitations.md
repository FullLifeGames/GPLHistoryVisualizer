...
- **Die Season Story erzählt aus Daten, nicht aus Prosa.** Alle Sätze sind i18n-Templates über `matches.csv`, die rekonstruierte Tabellenhistorie (gleiche Regeln wie die Zeitreise), `match_highlights.csv` und `title_odds.csv`; der „rechnerisch entschieden"-Beat ist als Simulation gebadged, und jeder Beat zitiert seine Quellzeilen. Season Story und GPL Wrapped sind eigene Menüpunkte der Saisons-Gruppe (zusätzlich von der Saisonakte verlinkt); die Zeitreise verlinkt nicht (sie hat keine gewählte Saison). Die Award-Beats folgen den `awards.csv`-Regeln und sind berechnet, nicht offiziell.
