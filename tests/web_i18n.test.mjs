...
assert.equal(t("de", "nav.audienceHistory"), "Publikum");
assert.equal(t("en", "nav.audienceHistory"), "Audience");
assert.equal(t("de", "sections.audienceHistoryTitle"), "Publikums-Geschichte");
assert.match(t("de", "sections.audienceHistoryDescription"), /Upload/);
assert.equal(t("de", "audience.metricUploads"), "Uploads pro Monat");
assert.equal(t("de", "audience.metricViews"), "Aufrufe pro Monat");
assert.equal(t("de", "audience.otherChannels"), "Andere");
assert.equal(t("en", "audience.metricViews"), "Views per month");
assert.equal(t("de", "audience.metricLikes"), "Likes pro Monat");
assert.equal(t("de", "audience.metricComments"), "Kommentare pro Monat");
assert.equal(t("de", "audience.anomaliesTitle"), "Interaktions-Anomalien");
assert.match(t("de", "audience.anomaliesNotes.uploads"), /Median/);
assert.match(t("de", "audience.anomaliesNotes.views"), /Median/);
assert.match(t("de", "audience.anomaliesNotes.likes"), /Like-Rate/);
assert.match(t("en", "audience.anomalyRate"), /\{median\}/);
assert.equal(t("de", "audience.groupBy"), "Aufschlüsselung");
assert.equal(t("de", "audience.groupType"), "Nach Kategorie");
assert.equal(t("de", "audience.allChannels"), "Alle Kanäle");
assert.match(t("de", "audience.stripNotes.engagement"), /Engagement/);
assert.match(t("en", "audience.stripNotes.views"), /z-scores/);
assert.equal(t("de", "nav.zeitstrahl"), "Meilensteine");
assert.equal(t("en", "nav.zeitstrahl"), "Milestones");
assert.equal(t("de", "sections.zeitstrahlTitle"), "Meilensteine");
assert.equal(t("de", "nav.zeitreise"), "Chronik");
assert.equal(t("en", "nav.zeitreise"), "Chronicle");
assert.equal(t("de", "navGroups.records"), "Bestenlisten");
assert.equal(t("de", "navGroups.data"), "Werkstatt");
assert.equal(t("de", "seasonHub.tab"), "Saison");
assert.equal(t("de", "seasonHub.overview"), "Akte");
assert.equal(t("en", "seasonHub.tab"), "Season");
assert.equal(t("de", "nav.wegweiser"), "Wegweiser");
assert.equal(t("en", "nav.wegweiser"), "Guide");
assert.match(t("de", "wegweiser.seasonHub"), /sechs Kapiteln/);
assert.match(t("de", "sections.zeitstrahlDescription"), /Upload/);
assert.equal(t("de", "zeitstrahl.sliderTitle"), "Was geschah am …?");
assert.match(t("de", "zeitstrahl.tickTitle"), /\{count\}/);
assert.match(t("de", "zeitstrahl.seasonEndChampion"), /\{champion\}/);
assert.match(t("de", "zeitstrahl.milestone"), /\{n\}/);
assert.match(t("en", "zeitstrahl.record"), /\{holder\}/);
...
assert.match(t("de", "filters.dataModeHint"), /Liga 2/);
assert.match(t("de", "filters.dataModeHint"), /Gesamtübersicht/);
assert.equal(t("de", "dataModes.primary"), "Gesamtübersicht (ohne Liga 2)");
assert.equal(t("de", "dataModes.all"), "Gesamtübersicht + Liga 2");
assert.equal(t("de", "dataModes.leagueTwo"), "Liga 2");
assert.equal(t("en", "dataModes.primary"), "Overall (without League 2)");
assert.equal(t("en", "dataModes.all"), "Overall + League 2");
assert.match(t("en", "filters.dataModeHint"), /League 2/);
assert.match(t("en", "filters.dataModeHint"), /Overall/);
assert.equal(t("de", "tableUx.moreColumns"), "Weitere Spalten rechts");
assert.equal(t("de", "tableUx.scrollLeft"), "Nach links");
assert.equal(t("de", "tableUx.scrollRight"), "Nach rechts");
assert.equal(t("en", "tableUx.moreColumns"), "More columns to the right");
assert.equal(t("en", "tableUx.scrollLeft"), "Scroll left");
assert.equal(t("en", "tableUx.scrollRight"), "Scroll right");
assert.equal(t("de", "statuses.sheet_extracted_with_user_correction"), "Sheet + Korrektur");
assert.equal(t("en", "statuses.sheet_extracted_with_user_correction"), "Sheet + correction");
...
assert.match(t("de", "columnHints.appearances"), /Killlisten/);
assert.match(t("de", "columnHints.appearances"), /Datenlücken/);
assert.equal(t("de", "columnHints.appearances"), t("de", "columnHints.kills"));
assert.match(t("de", "columnHints.kill_rate"), /Kills \/ Einsätze/);
assert.equal(t("de", "columnHints.kill_rate"), t("de", "columnHints.appearances"));
assert.match(t("en", "columnHints.kills"), /killlists/);
assert.match(t("en", "columnHints.kills"), /data gaps/);
assert.match(t("en", "columnHints.kill_rate"), /Kills \/ appearances/);
...
assert.equal(columnTitle("de", "kill_rate"), "Killquote");
